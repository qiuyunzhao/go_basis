...
func main() {
	...
	for i := 0; i < 8; i++ {
		<-exitChan // 协程读取是阻塞式的，还有协程会写入数据，但当前没有可以拿出的数据就阻塞等待
	}
	...
	//	fmt.Println("使用协程耗时=", end-start)
	//
	//	//当我们从exitChan取到了8个结束标志，说明获取素数的协程都执行完了，关闭 primeChan
	//	close(primeChan)
	//}()
...
func primeNum(intChan chan int, primeChan chan int, exitChan chan bool, routineId int) {
	var flag bool //素数标志
	...
	fmt.Println("第", routineId, "个primeNum 协程因为取不到数据，退出")
