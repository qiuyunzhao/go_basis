...
func (c *ConfSimple) getConfSimple() {
	yamlFile, err := ioutil.ReadFile("36_读取配置文件/1_yaml配置文件/conf.yaml")
	if err != nil {
...
func (c *Myconf) getConfStruct() {
	yamlFile, err := ioutil.ReadFile("36_读取配置文件/1_yaml配置文件/conf1.yaml")
	if err != nil {
		fmt.Println(err.Error())
	}
	err = yaml.Unmarshal(yamlFile, c)
...
func (c *Conf) getConf() {
	yamlFile, err := ioutil.ReadFile("36_读取配置文件/1_yaml配置文件/conf2.yaml")
	if err != nil {
		fmt.Println(err.Error())
	}
